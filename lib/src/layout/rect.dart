...
final class Rect {
  ...
  /// Returns a smaller rectangle inside this rectangle with the given [margin].
  Rect withMargin(Margin margin) {
    return Rect(
      x + margin.left,
      y + margin.top,
      width - margin.right,
  ...
  Rect withOffset(Offset offset) => Rect(offset.x, offset.y, width, height);
...
final class _RectRowsIterator implements Iterator<Rect> {
  ...
  bool moveNext() {
    ...
    _current = Rect(_rect.x, y, _rect.width, 1);
    _index++;
    return true;
  }
}
...
final class _RectColumnsIterator implements Iterator<Rect> {
  ...
  bool moveNext() {
    ...
    _current = Rect(x, _rect.y, 1, _rect.height);
    _index++;
    return true;
  }
}
